...
def download_batch(datasets,
    ...
    with FTP(CEDA_FTP) as ftp:
        ...
        for i, dataset in enumerate(datasets):
            ...
            #a=path_on_ftp.replace('\\','/') + "/"
			#读取路径下的文件
            filelist = ftp.nlst(path_on_ftp.replace('\\','/') + "/")
            #filelist = ftp.nlst('/badc/cmip5/data/cmip5/output1/BCC/bcc-csm1-1/sstClim/mon/atmos/Amon/r1i1p1/latest/rlutcs/')
            print(filelist)
            for fn in filelist:
